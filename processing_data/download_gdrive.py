...
def txt2csv(filepath_txt, filepath_folder, name):
    data = pd.read_csv(filepath_txt, sep=",", header=None, error_bad_lines=False)
    # print(data)
    # start_column = 1
    ...
    if "Head" in name:
        ...
        data.to_csv('demo/Head.csv', index=False)
    elif "Chest" in name:
        data.to_csv(filepath_folder + '/' + name + '.csv', index=False)
        data.to_csv('demo/Chest.csv', index=False)
    else:
        data.to_csv(filepath_folder + '/' + name + '.csv', index=False)
        data.to_csv('demo/Gun.csv', index=False)
