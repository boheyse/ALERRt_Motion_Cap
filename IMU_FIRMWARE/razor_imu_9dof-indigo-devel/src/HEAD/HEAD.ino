...
float magn_ellipsoid_center[3] = {489.625, -189.767, -103.640};
float magn_ellipsoid_transform[3][3] = {{0.998488, -0.00401357, 0.00710655}, {-0.00401357, 0.972644, 0.00518907}, {0.00710655, 0.00518907, 0.955412}};
...
void loop()
{
  while(Serial1.available())
  {
    ...
    if(toSend == start)
    {
      ...
      //Serial1.println("Recording Head");
      while(runn)
      {
    ...
    if(toSend == beginTransfer){
      tsfr = true;
      SerialUSB.println("Got Transfer Signal");
      delay(2000);
    }
    
    int delay_count = 0;
    if(tsfr){
      Serial1.println("\nTransferring Data from: Head\n");
