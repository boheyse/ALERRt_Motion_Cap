...
#define OUTPUT__DATA_INTERVAL 25  // in milliseconds
...
float magn_ellipsoid_center[3] = {-8.99585, -16.0099, -346.453};
float magn_ellipsoid_transform[3][3] = {{0.980203, -0.00355000, 0.0443217}, {-0.00355000, 0.943862, 0.000359715}, {0.0443217, 0.000359715, 0.899733}};
...
#define DEBUG__USE_DEFAULT_GYRO_FSR_M0 true

#define DEBUG__USE_DEFAULT_ACCEL_FSR_M0 true
...
/////////////////////////////////////////////Added code by TXST 2.10 Fall 2019/////////////////////////////////
...
void CheckBatLevel(void)
{
  ...
  LipoV = map(LipoV,710,920,0,100);   // convert analog read values to percentage
  Serial1.print("BattG:" + String(LipoV));
}
...
void loop()
{
  while(Serial1.available())
  {
    ...
    if(toSend == start)
    {
      timestamp_high = gpstimestamp.substring(0,6);
      //Serial1.println(timestamp_high);
      timestamp_low = gpstimestamp.substring(6,13).toInt();
      //Serial1.println(timestamp_low);
      gpstimestamp = "";
      ...
      //Allow user to put phone into pocket to begin recording
      delay(1000);
      
      sdLogString("GunGun\n\n");
      if (gpstimestamp.length() == 13){
        CheckBatLevel();
      }
      //Serial1.println("SD card is recording");
      while(runn)
      {
    ...
    if(toSend == beginTransfer){
      tsfr = true;
      SerialUSB.println("Starting transfer");
      delay(2000);
    }
    ...
    int delay_count = 0;
    if(tsfr){
      Serial1.println("Transferring Data from: Gun\n");
